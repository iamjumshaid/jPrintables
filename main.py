...
pdf_files = [file for file in os.listdir(printables_folder) if file.endswith(".pdf")]

print("Files in 'printables' folder:")
for file in pdf_files:
    print(file)

user_input = input("Are these the files that you wish to print? (yes/no): ").lower()
...
if user_input == "yes":
    ...
    for file in pdf_files:
        ...
        lpr_command = f"lpr -o sides=one-sided {file} -P {PRINTER_NAME}"

        subprocess.run(ssh_command + lpr_command, shell=True, check=True)

        lpq_command = f"{ssh_command} lpq -P {PRINTER_NAME}"
        lpq_result = subprocess.run(lpq_command, shell=True, capture_output=True, text=True)
